  ...
  setup() {
    ...
    const checkTodayStatus = async () => {
      try {
        ...
        if (!att || !att.check_in_time) { pageState.value = 'no_checkin'; return }
        ...
        await initFace()
      } catch (_) { pageState.value = 'no_checkin' }
    }
    ...
    const startCamera = async () => {
      try {
        ...
        let waited = 0
        while (!videoEl.value && waited < 3000) { await new Promise(r => setTimeout(r, 50)); waited += 50 }
        if (!videoEl.value) { errorMsg.value = 'Kamera tidak bisa dimuat.'; return }
        videoEl.value.srcObject = mediaStream
        await new Promise<void>(res => { videoEl.value!.onloadedmetadata = () => res(); setTimeout(res, 3000) })
        await videoEl.value.play()
        camStatus.value = 'detecting'
        startDetectLoop()
      } catch (e: any) { errorMsg.value = 'Gagal mengakses kamera: ' + (e?.message ?? '') }
    }
    ...
    const startDetectLoop = () => {
      ...
      detectInterval = window.setInterval(async () => {
        ...
        const canvas = canvasEl.value; const video = videoEl.value
        if (canvas && video) {
        ...
        if (det && !autoTimer && !processing.value) {
          autoTimer = window.setTimeout(() => { autoTimer = null; if (camStatus.value === 'ready') doCheckout() }, 1500)
        }
    ...
    const doCheckout = async () => {
      ...
      try {
        ...
        if (!det) {
          camStatus.value = 'no_face'; errorMsg.value = 'Wajah tidak terdeteksi. Coba lagi.'
        ...
        if (!bestMatch || bestDist > THRESHOLD) {
          camStatus.value = 'failed'; errorMsg.value = 'Wajah tidak dikenali. Pastikan pencahayaan cukup.'
          processing.value = false
          setTimeout(() => { camStatus.value = 'detecting'; errorMsg.value = '' }, 2500); return
        }

        if (bestMatch.user_id !== authStore.user.id) {
          camStatus.value = 'failed'; errorMsg.value = 'Wajah tidak cocok dengan akun yang sedang login.'
          processing.value = false
          setTimeout(() => { camStatus.value = 'detecting'; errorMsg.value = '' }, 2500); return
        }
    ...
    const startCountdown = () => {
      ...
      const timer = setInterval(() => {
        countdown.value--
        if (countdown.value <= 0) { clearInterval(timer); authStore.logout(); router.push({ name: 'sign-in' }) }
      }, 1000)
    }
