      ...
      <table class="ta-table">
        ...
        <tbody>
          ...
          <tr v-for="item in attendances" :key="item.id">
            ...
            <td>
              <div class="ta-user">
                <!-- avatar sudah URL lengkap dari backend -->
    ...
    const viewPhoto = async (attendanceId: number, type: 'in' | 'out') => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE}/attendance/photo/${attendanceId}/${type}`,
...
.ta-btn-photo { background: rgba(27,132,255,0.15); color: #1b84ff; border: 1px solid rgba(27,132,255,0.2); border-radius: 8px; padding: 4px 10px; font-size: 12px; cursor: pointer; transition: background 0.15s; white-space: nowrap; }
...
.ta-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 9999; }
.ta-modal { background: #15171e; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; width: 90%; max-width: 480px; overflow: hidden; }
...
.ta-modal-close { background: none; border: none; color: #55555e; font-size: 16px; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: color 0.15s; }
.ta-modal-close:hover { color: #f0f0f5; }
.ta-modal-body { padding: 20px; display: flex; justify-content: center; align-items: center; min-height: 200px; }
