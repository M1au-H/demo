<template>
  <div class="fm-wrap">
    ...
    <div v-else class="fm-grid">
      <div v-for="emp in employees" :key="emp.id" class="fm-card" :class="{ 'has-face': emp.has_face }">
        <div class="fm-avatar-wrap">
          ...
          <div class="fm-face-badge" :class="emp.has_face ? 'registered' : 'empty'">
            <svg v-if="emp.has_face" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
        ...
        <div class="fm-card-info">
          ...
          <div v-if="emp.has_face && emp.enrolled_at" class="fm-enrolled-at">Didaftarkan: {{ emp.enrolled_at }}</div>
    ...
    <div v-if="enrollModal.show" class="fm-modal-overlay" @click.self="closeEnroll">
      <div class="fm-modal">
        ...
        <div class="fm-modal-body">
          ...
          <!-- Instruksi auto-capture -->
          <div class="fm-auto-tips">
            <div class="fm-tip-icon">🤖</div>
            <div>
              <strong>Mode Otomatis</strong> — Gerakkan kepala perlahan ke kiri, kanan, atas, bawah.<br/>
              <span class="fm-tip-sub">Sistem akan otomatis mengambil sample saat posisi wajah berubah.</span>
            </div>
          </div>
          ...
          <div class="fm-enroll-camera" :class="enrollStatus">
            ...
            <!-- Flash saat capture -->
            <div v-if="captureFlash" class="fm-capture-flash"></div>
            ...
            <div class="fm-enroll-badge" :class="enrollStatus">
              ...
              <span v-else-if="enrollStatus === 'ready'">✅ Gerakkan kepala untuk capture otomatis</span>
              <span v-else-if="enrollStatus === 'no_face'">👤 Arahkan wajah ke kamera</span>
              <span v-else-if="enrollStatus === 'captured'">📸 Sample diambil!</span>
            </div>
          </div>

          <!-- Progress -->
          <div class="fm-sample-progress">
            <div class="fm-sample-label">
              Sample terkumpul:
              <strong>{{ samples.length }} / {{ REQUIRED_SAMPLES }}</strong>
              <span v-if="samples.length >= REQUIRED_SAMPLES" class="fm-label-done">✓ Siap disimpan</span>
            </div>
            <div class="fm-sample-bar">
              <div class="fm-sample-fill" :style="{ width: (samples.length / REQUIRED_SAMPLES * 100) + '%' }"></div>
            </div>
            <!-- Titik indikator posisi -->
            <div class="fm-sample-dots">
              <div
                v-for="i in REQUIRED_SAMPLES"
                :key="i"
                class="fm-dot"
                :class="i <= samples.length ? 'filled' : ''"
              ></div>
            </div>
          </div>
          ...
          <!-- Auto-save status -->
          <div v-if="saving" class="fm-autosave-status">
            <span class="fm-spinner-sm"></span>
            Menyimpan data wajah secara otomatis...
          </div>
          <div v-else-if="samples.length < REQUIRED_SAMPLES" class="fm-autosave-hint">
            ⚡ Akan otomatis tersimpan setelah {{ REQUIRED_SAMPLES }} sample terkumpul
          </div>

          <!-- Tombol Ulangi (tetap ada jika ada error/duplikat) -->
          <button v-if="samples.length > 0 && !saving" class="fm-btn-reset" @click="resetSamples">
...
// Jarak minimum antar sample agar dianggap posisi berbeda
// Makin kecil = makin sensitif (lebih mudah capture)
const MIN_MOVE_THRESHOLD = 0.018
...
export default defineComponent({
  ...
  setup() {
    ...
    const enrollStatus     = ref<'idle' | 'detecting' | 'ready' | 'no_face' | 'captured'>('idle')
    ...
    // Descriptor terakhir yang berhasil di-capture (untuk cek apakah posisi sudah bergerak)
    let lastCapturedDescriptor: Float32Array | null = null
    // Cooldown agar tidak capture terlalu cepat
    ...
    const checkDuplicate = (descriptor: Float32Array): string => {
      ...
      for (const profile of allProfiles) {
        ...
        if (dist < DUPLICATE_THRESHOLD) return profile.name
      }
      return ''
    }
    ...
    const startEnrollDetect = () => {
      detectInterval = window.setInterval(async () => {
        ...
        if (!det) {
          enrollStatus.value = 'no_face'
          drawEnrollOverlay(null)
          return
        }

        enrollStatus.value = 'ready'
        drawEnrollOverlay(det)

        // ── AUTO CAPTURE saat posisi wajah berubah ──
        if (captureCooldown) return
        if (duplicateWarning.value) return

        const descriptor = det.descriptor
        const shouldCapture = !lastCapturedDescriptor ||
          euclidean(Array.from(descriptor), Array.from(lastCapturedDescriptor)) > MIN_MOVE_THRESHOLD

        if (!shouldCapture) return

        // Cek duplikat dengan user lain
        const dupName = checkDuplicate(descriptor)
        if (dupName) {
        ...
        duplicateWarning.value = ''
        enrollError.value      = ''
        samples.value          = [...samples.value, descriptor]
        lastCapturedDescriptor = descriptor

        // ✅ AUTO SAVE saat sample sudah cukup
        if (samples.value.length >= REQUIRED_SAMPLES) {
          if (detectInterval) { clearInterval(detectInterval); detectInterval = null }
          setTimeout(() => saveEnrollment(), 400)
        }
        ...
        // Status captured sebentar
        enrollStatus.value = 'captured'
        setTimeout(() => {
          if (enrollStatus.value === 'captured') enrollStatus.value = 'ready'
        }, 400)

        // Cooldown 800ms agar tidak capture terlalu cepat berturut-turut
    ...
    const drawEnrollOverlay = (det: any) => {
      ...
      ctx.strokeStyle = enrollStatus.value === 'captured' ? '#1b84ff' : '#17c653'
      ctx.lineWidth   = 2.5
    ...
    const confirmDelete = async (emp: any) => {
      const result = await Swal.fire({
        ...
        text: `Data wajah ${emp.name} akan dihapus.`,
    ...
    return {
      ...
      samples, REQUIRED_SAMPLES, captureFlash,
      enrollVideo, enrollCanvas,
      openEnroll, closeEnroll,
      resetSamples, saveEnrollment,
      confirmDelete,
    }
  }
})
...
.fm-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; flex-wrap: wrap; gap: 16px; }
...
.fm-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 14px; }
.fm-card { background: #15171e; border: 1px solid rgba(255,255,255,0.06); border-radius: 14px; padding: 18px; display: flex; flex-direction: column; gap: 12px; transition: border-color 0.2s, box-shadow 0.2s; }
...
.fm-face-badge { position: absolute; bottom: 0; right: 0; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid #15171e; }
...
.fm-btn-enroll { display: flex; align-items: center; justify-content: center; gap: 6px; background: rgba(27,132,255,0.12); color: #5aabff; border: 1px solid rgba(27,132,255,0.2); border-radius: 8px; padding: 9px 14px; font-size: 12.5px; font-weight: 600; cursor: pointer; transition: all 0.15s; width: 100%; }
.fm-btn-enroll:hover { background: rgba(27,132,255,0.22); border-color: rgba(27,132,255,0.4); }
.fm-btn-register { background: rgba(23,198,83,0.1); color: #17c653; border-color: rgba(23,198,83,0.2); }
.fm-btn-register:hover { background: rgba(23,198,83,0.2); border-color: rgba(23,198,83,0.4); }
.fm-btn-delete { display: flex; align-items: center; justify-content: center; gap: 6px; background: rgba(255,107,107,0.08); color: #ff6b6b; border: 1px solid rgba(255,107,107,0.15); border-radius: 8px; padding: 9px 14px; font-size: 12.5px; font-weight: 600; cursor: pointer; transition: all 0.15s; width: 100%; }
.fm-btn-delete:hover { background: rgba(255,107,107,0.18); }

/* Modal */
.fm-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.85); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px; }
.fm-modal { background: #15171e; border: 1px solid rgba(255,255,255,0.08); border-radius: 18px; width: 100%; max-width: 500px; overflow: hidden; max-height: 90vh; overflow-y: auto; }
.fm-modal-header { display: flex; justify-content: space-between; align-items: center; padding: 18px 22px; border-bottom: 1px solid rgba(255,255,255,0.06); position: sticky; top: 0; background: #15171e; z-index: 1; }
...
/* Tips auto */
.fm-auto-tips { display: flex; align-items: flex-start; gap: 10px; padding: 12px 14px; background: rgba(27,132,255,0.06); border: 1px solid rgba(27,132,255,0.15); border-radius: 10px; font-size: 12.5px; color: #aaaabc; line-height: 1.5; }
.fm-tip-icon { font-size: 20px; flex-shrink: 0; }
.fm-auto-tips strong { color: #5aabff; }
.fm-tip-sub { color: #55555e; font-size: 11.5px; }

/* Camera */
.fm-enroll-camera { position: relative; border-radius: 14px; overflow: hidden; aspect-ratio: 4/3; background: #0a0c10; border: 2px solid rgba(255,255,255,0.07); transition: border-color 0.3s, box-shadow 0.3s; }
.fm-enroll-camera.ready    { border-color: #17c653; box-shadow: 0 0 20px rgba(23,198,83,0.2); }
.fm-enroll-camera.captured { border-color: #1b84ff; box-shadow: 0 0 20px rgba(27,132,255,0.3); }
...
.fm-capture-flash { position: absolute; inset: 0; background: rgba(255,255,255,0.3); animation: flash 0.2s ease forwards; pointer-events: none; z-index: 10; }
...
.fm-enroll-badge { position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); background: rgba(10,12,16,0.85); backdrop-filter: blur(6px); border: 1px solid rgba(255,255,255,0.07); border-radius: 20px; padding: 5px 12px; font-size: 11.5px; font-weight: 600; color: #aaaabc; white-space: nowrap; }
.fm-enroll-badge.ready    { color: #17c653; border-color: rgba(23,198,83,0.3); }
.fm-enroll-badge.captured { color: #1b84ff; border-color: rgba(27,132,255,0.3); }
.fm-enroll-badge.no_face  { color: #ffa500; border-color: rgba(255,165,0,0.3); }

/* Progress */
.fm-sample-progress { }
.fm-sample-label { font-size: 12px; color: #55555e; margin-bottom: 6px; display: flex; align-items: center; gap: 8px; }
.fm-sample-label strong { color: #f0f0f5; }
.fm-label-done { color: #17c653; font-weight: 700; font-size: 11px; }
.fm-sample-bar { height: 6px; background: rgba(255,255,255,0.07); border-radius: 6px; overflow: hidden; margin-bottom: 8px; }
.fm-sample-fill { height: 100%; background: linear-gradient(90deg, #e8262a, #ff6b6b); border-radius: 6px; transition: width 0.3s; }
.fm-sample-dots { display: flex; gap: 6px; justify-content: center; }
.fm-dot { width: 10px; height: 10px; border-radius: 50%; background: rgba(255,255,255,0.08); border: 1.5px solid rgba(255,255,255,0.12); transition: all 0.25s; }
.fm-dot.filled { background: #17c653; border-color: #17c653; box-shadow: 0 0 6px rgba(23,198,83,0.5); }

.fm-duplicate-box { display: flex; align-items: flex-start; gap: 8px; padding: 12px 14px; border-radius: 10px; background: rgba(255,165,0,0.08); border: 1px solid rgba(255,165,0,0.25); color: #ffa500; font-size: 12.5px; line-height: 1.5; }
...
.fm-btn-reset { display: flex; align-items: center; justify-content: center; gap: 6px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #aaaabc; border-radius: 10px; padding: 11px 16px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
.fm-btn-reset:hover:not(:disabled) { background: rgba(255,255,255,0.09); color: #f0f0f5; }
.fm-btn-reset:disabled { opacity: 0.35; cursor: not-allowed; }
.fm-btn-save { flex: 1; display: flex; align-items: center; justify-content: center; gap: 6px; background: #e8262a; color: #fff; border: none; border-radius: 10px; padding: 11px; font-size: 13px; font-weight: 700; cursor: pointer; transition: background 0.15s; }
...
.fm-autosave-status { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; background: rgba(27,132,255,0.08); border: 1px solid rgba(27,132,255,0.2); border-radius: 10px; color: #5aabff; font-size: 13px; font-weight: 600; }
.fm-autosave-hint { text-align: center; font-size: 12px; color: #3a3a48; padding: 8px; }
